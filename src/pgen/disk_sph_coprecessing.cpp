...
//
// Febuary 2026, Ted Johnson, Work in the co-precessing frame of the disk to reduce the
//                size of the domain.
//
...
static Real timeout=0.0;
// planetary system: circumplanetary disk depletion

// disk inclination
static Real diskinc;
// Additional coordinate system inclination
static Real coordinc;
// Viscosity
static Real alpha;
...
// This assumes the disk with the highest z-value is on the negative y axis,
// and the midplane intersects the z-axis.
// In other words, the vector points to the first quadrant of the y-z plane.
// In this coordinate system, the angle is between the z-axis and the normal,
// Which is not the same as the inclination. It is the variable `coordinc`.
...
LogLevel str_to_loglevel(const std::string& level) {
  ...
  else {throw std::runtime_error("Log level "+level+" not recognized");}
}

std::string loglevel_to_str(const LogLevel& level) {
  switch (level)
  {
...
const char* loglevel_to_color(const LogLevel& level) {
  switch (level)
  {
...
void Logger::trace(const std::string& msg) {
  write_log(LogLevel::TRACE,msg,__FILE__, __LINE__);
}
void Logger::info(const std::string& msg) {
  write_log(LogLevel::INFO,msg, __FILE__, __LINE__);
}
void Logger::warning(const std::string& msg) {
  write_log(LogLevel::WARNING,msg, __FILE__, __LINE__);
}
void Logger::error(const std::string& msg) {
  write_log(LogLevel::ERROR,msg, __FILE__, __LINE__);
}
void Logger::write_log(
  ...
  if (level >= _level) { // otherwise don't do anything
    ...
    std::string _src = std::string(file) + ":" + std::to_string(lineno);
    std::cout
      ...
      << std::setw(40) << _src
      << " | "
      << loglevel_to_color(level) << std::setw(7) << loglevel_to_str(level) << "\033[0m"
      << " | "
      << msg
      << "\n";
  }
...
static Logger logger = Logger(LogLevel::TRACE);
...
double solve_newton(
  ...
  auto stop = [func, grad, interval](double x)->bool {
    if (func(x) < 0.0) {
      if (grad(x) > 0.0) { return func(x+interval) > 0.0;}
      else {return func(x-interval) > 0.0;}
    }
    else {
      if (grad(x) < 0.0) {return func(x+interval) < 0.0;}
      else {return func(x-interval) <  0.0;}
  ...
  double val = guess;
  int niter = 0;
  logger.trace("Starting Newton Solver with guess " + std::to_string(guess));
  while (stop(val) == false) {
    niter += 1;
    logger.trace("N = "+std::to_string(niter-1)+", val: " + std::to_string(val)+", func: " + std::to_string(func(val)) + ", grad: " + std::to_string(grad(val)));
    val = val - func(val) / grad(val);
    if (std::isnan(val)) {
      logger.error("Found NaN after " + std::to_string(niter) + " iterations!");
      throw std::runtime_error("Found NaN in Newton Solver!");
    }
    
  }
  logger.trace("Converged after " + std::to_string(niter) + " iterations");
  return val;
}
...
public:
  ...
  double ebin;
  double mean_anom;
  double arg_of_pariapsis;
  Vec3D ascending_node;
  Vec3D specific_anglular_momentum;
  double ecc_anom();
  double tru_anom();
  BinarySystem(
    double _mass, double _smax, double _ebin, double _mean_anom, double _arg_of_pariapsis,
    Vec3D _ascending_node, Vec3D _specific_anglular_momentum
  );
  ~BinarySystem()=default;
  void update(double time, double gm);
  Vec3D vecpos();
  bool needs_update(double time) {
...
BinarySystem::BinarySystem(
  ...
  double _ebin,
  double _mean_anom,
  double _arg_of_pariapsis,
  Vec3D _ascending_node,
  Vec3D _specific_angular_momentum
):
  mass(_mass),
  smax(_smax),
  ebin(_ebin),
  mean_anom(_mean_anom),
  arg_of_pariapsis(_arg_of_pariapsis),
  ascending_node(_ascending_node),
  specific_anglular_momentum(_specific_angular_momentum),
  mean_anom_init(_mean_anom)
{}

void BinarySystem::update(double time, double gm) {
  logger.trace("Updating secondary location");
  double orbper = sqrt(
    4.0 * PI * PI / gm * smax * smax * smax
  );
  double angle_orbited = 2.0 * PI * time / orbper;
  mean_anom = std::fmod(angle_orbited + mean_anom_init, 2.0 * PI);
  last_update = time;
...
double BinarySystem::ecc_anom() {
  double guess = mean_anom - ebin * sin(mean_anom);
  auto func = [this](double _ecc_anom)-> double {return mean_anom - _ecc_anom + ebin*sin(_ecc_anom);};
  auto grad = [this](double _ecc_anom)-> double {return -1.0 + ebin*cos(_ecc_anom);};
  return solve_newton(func, grad, guess);
}

double BinarySystem::tru_anom() {
  double _ecc_anom = ecc_anom();
  return 2.0 * atan(
    sqrt(
      (1.0+ebin) / (1.0-ebin)
    )
...
Vec3D BinarySystem::vecpos() {
  Vec3D hhat = specific_anglular_momentum;
  hhat = hhat * (1/hhat.magnitude());
  Vec3D asc_node = ascending_node;
  asc_node = asc_node * (1/asc_node.magnitude());
  double nu = tru_anom();
  double omega = arg_of_pariapsis;
  Vec3D first_term = (asc_node * cos(omega) + cross(hhat,asc_node)*sin(omega)) * cos(nu);
  Vec3D second_term = cross(
    hhat,
    asc_node*cos(omega) + cross(hhat,asc_node)*sin(omega)
  ) * sin(nu);
  return (first_term + second_term) * smax;
}
...
void Mesh::InitUserMeshData(ParameterInput *pin)
{
  ...
  loglevel = str_to_loglevel(pin->GetOrAddString("problem","log","warning"));
  ...
  psys = new BinarySystem(
    pin->GetReal("secondary", "mass"),
    pin->GetReal("secondary", "smax"),
    pin->GetReal("secondary", "ebin"),
    pin->GetReal("secondary", "mean_anom"),
    pin->GetReal("secondary", "argp"),
    Vec3D(0.0,cos(diskinc-coordinc),sin(diskinc-coordinc)),
    Vec3D(0.0,-sin(diskinc-coordinc),cos(diskinc-coordinc))
  );

  // for planetary orbit output
  if(Globals::my_rank==0) myfile.open("orbit.txt",std::ios::out);
  if (myfile.is_open()&&Globals::my_rank==0) {
    myfile 
    << std::setw(18) << "time"
    << std::setw(18) << "x"
...
static Real rho_floorsf(const Real x1, const Real x2, const Real x3)
{
  ...
  Real r = get_midplane_projection_distance(v,coordinc);
  Real z = fabs(get_z_above_midplane(v,coordinc));
...
{  
  ...
  if (vflag == 2 || vflag == 21 || vflag == 22) {       
    ...
    if (den <= rho_floorsf(x1, x2, x3)) { // probably not
      ...
      if (NON_BAROTROPIC_EOS){
        ...
        if(vflag==22){ // yes
          ...
          Vec3D nhat = get_nhat_tilt(coordinc);
          Vec3D grad_pressure_z = nhat * dot(nhat,grad_pressure);
          Vec3D grad_pressure_r = grad_pressure - grad_pressure_z;
          Vec3D position_on_midplane = r_perpendicular(Vec3D::FromSph(x1,x2,x3), coordinc);
    ...
    Vec3D nhat = get_nhat_tilt(coordinc);
    Vec3D r_perp = r_perpendicular(v,coordinc);
   ...
   for(int k=pmb->ks; k<=pmb->ke; ++k){
     for (int j=pmb->js; j<=pmb->je; ++j) {
       ...
       for (int i=pmb->is; i<=pmb->ie; ++i) {
        ...
        Real r = get_midplane_projection_distance(rvec,coordinc);
        Real z = get_z_above_midplane(rvec,coordinc);
...
void MeshBlock::UserWorkInLoop() {
  if (psys->needs_update(pmy_mesh->time)) {
    psys->update(pmy_mesh->time,gm_primary);
  }
  else {
    logger.trace("Secondary update skipped in Block " + std::to_string(gid) + ".");
  }
}